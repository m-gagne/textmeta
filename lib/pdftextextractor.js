          ...
          return document.getPage(pageNumber).then((page) => {
            ...
            return page.getTextContent().then((content) => {
              content.items.map((item) => {
                ...
                isAllWhitespace = this.isAllWhitespace(item.str);

                if (documentText !== "") {
                  if (lastItem && this.applyPrecision(item.appliedTransform[1]) != this.applyPrecision(lastItem.appliedTransform[1])) {
                    documentText += "\n";
                  } else if (!isAllWhitespace && lastItem &&
                    this.applyPrecision(lastItem.appliedTransform[0] + lastItem.width) < this.applyPrecision(item.appliedTransform[0])
                  ){
                    documentText += " ";
                  }
                }
